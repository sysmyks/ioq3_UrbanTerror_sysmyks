...
ifneq ($(BUILD_SERVER),0)
  TARGETS += $(B)/SysmyksMod.$(ARCH)$(BINEXT)
endif
...
$(B)/SysmyksMod.$(ARCH)$(BINEXT): $(Q3DOBJ)
	$(echo_cmd) "LD $@"
...
ifneq ($(BUILD_SERVER),0)
	@if [ -f $(BR)/SysmyksMod.$(ARCH)$(BINEXT) ]; then \
		$(INSTALL) -s -m 0755 $(BR)/SysmyksMod.$(ARCH)$(BINEXT) $(COPYDIR)/SysmyksMod.$(ARCH)$(BINEXT); \
	fi
endif
