...
#include "server.h"
// Variables globales pour sauvegarder la position et les angles
static vec3_t savedPosition; // Position sauvegardée : x, y, z
static vec3_t savedAngles;   // Angles sauvegardés : pitch, yaw, roll
...
static void SV_SavePlayerPos_f(void) {
    ...
    if (!client) {
        Com_Printf("Invalid player number specified.\n");
        return;
    }
    ...
    // Sauvegarder la position et l'orientation
    VectorCopy(ps->origin, savedPosition);
    ...
    Com_Printf("Player %d position saved: (%.2f, %.2f, %.2f) - Angles: (%.2f, %.2f, %.2f)\n",
               ...
               savedAngles[0], savedAngles[1], savedAngles[2]);

    // Ouvrir le fichier pour écrire
    fileHandle_t playerLogFile = FS_FOpenFileWrite("player_positions.log");
    if (!playerLogFile) {
        Com_Printf("Failed to open player_positions.log for writing.\n");
        return;
    }
    ...
    Com_sprintf(logEntry, sizeof(logEntry),
                "Player %d position saved: (%.2f, %.2f, %.2f) - Angles: (%.2f, %.2f, %.2f)\n",
                clientNum,
                savedPosition[0], savedPosition[1], savedPosition[2],
                savedAngles[0], savedAngles[1], savedAngles[2]);

    FS_Write(logEntry, strlen(logEntry), playerLogFile);
    FS_Flush(playerLogFile); // Flusher les données
    FS_FCloseFile(playerLogFile); // Fermer le fichier
}
...
static void SV_LoadPlayerPos_f(void) {
    ...
    if (Cmd_Argc() != 7) {
        Com_Printf("Usage: loadplayerpos <player-num> <x> <y> <z> <pitch> <yaw>\n");
        return;
    }

    // Récupère le joueur par son numéro
    client = SV_GetPlayerByNum();
    if (!client) {
        Com_Printf("Invalid player number specified.\n");
        return;
    ...
    // Obtiens l'entité et l'état du joueur
    clientNum = client - svs.clients; // Numéro du joueur
    ent = SV_GentityNum(clientNum);  // Entité associée
    ps = SV_GameClientNum(clientNum); // État du joueur

    // Récupère la position et les angles depuis les arguments de la commande
    targetPosition[0] = atof(Cmd_Argv(2));  // x
    targetPosition[1] = atof(Cmd_Argv(3));  // y
    targetPosition[2] = atof(Cmd_Argv(4));  // z

    targetAngles[0] = atof(Cmd_Argv(5)); // Pitch
    targetAngles[1] = atof(Cmd_Argv(6)); // Yaw
    targetAngles[2] = 0; // Roll est toujours 0 pour Urban Terror

    // Téléporte le joueur à la nouvelle position
    VectorCopy(targetPosition, ps->origin);
    VectorClear(ps->velocity); // Réinitialise la vélocité

    // Applique les angles de vue
    for (int i = 0; i < 2; i++) {
        int angle = ANGLE2SHORT(targetAngles[i]);
        ps->delta_angles[i] = angle - client->lastUsercmd.angles[i];
    }

    // Met à jour l'entité avec les nouveaux angles
    VectorCopy(targetAngles, ent->s.angles);
    VectorCopy(targetAngles, ps->viewangles);

    // Affiche un message pour confirmer que le joueur a été téléporté
    Com_Printf("Player %d teleported to position (%.2f, %.2f, %.2f) with view angles (%.2f, %.2f)\n",
               clientNum,
...
void SV_InfiniteStamina_fc(void)
{
    ...
    if (!com_sv_running->integer) {
        Com_Printf("Debug: Server is not running.\n");
        return;
    }

    if (Cmd_Argc() != 2) { // Pas besoin d'argument supplémentaire à part le joueur
        Com_Printf("Usage: infinitestamina <player>\n");
        return;
    }

    cl = SV_GetPlayerByHandle();
    if (!cl) {
        Com_Printf("Error: Player not found.\n");
        return;
    }

    cid = cl - svs.clients;
    if (cid < 0 || cid >= sv_maxclients->integer) {
        Com_Printf("Error: Invalid client ID %d.\n", cid);
        return;
    }
    ...
    if (cl->cm.infiniteStamina == 1) {
        ...
        if (svs.clients[cid].cm.ready) {
            Com_Printf("^7Cannot enable infinite stamina in ready.\n");
            return;
        }
        cl->cm.infiniteStamina = 1;
        Com_Printf("^7Infinite stamina enabled.\n");
    }
}
...
void SV_InfiniteWallJumps_fc(void)
{
    ...
    if (Cmd_Argc() != 2) { // Pas besoin d'argument supplémentaire à part le joueur
        Com_Printf("^7Usage: infinitewalljumps <player>\n");
        return;
    }
    ...
    // Basculer entre 1 et 0
    if (cl->cm.infiniteWallJumps == 1) {
        // Si déjà activé, on le désactive
        cl->cm.infiniteWallJumps = 0;
        Com_Printf("^7Infinite wall jumps disabled.\n");
    } else {
        // Si désactivé, on vérifie d'abord le mode "ready"
        if (svs.clients[cid].cm.ready) {
            Com_Printf("^7Cannot enable infinite wall jumps in ready.\n");
            return;
        }

        // Sinon, on active les wall jumps infinis
        cl->cm.infiniteWallJumps = 1;
        Com_Printf("^7Infinite wall jumps enabled.\n");
    }
}
...
void SV_AddOperatorCommands( void ) {
    ...
    Cmd_AddCommand ("Serv_infinitestamina", SV_InfiniteStamina_fc);
    Cmd_AddCommand ("Serv_infinitewalljumps", SV_InfiniteWallJumps_fc);
